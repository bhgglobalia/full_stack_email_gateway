...
export class EventsService {
    ...
    async list(limit = 100, provider?: string, clientId?: string, date?: string) {
        let qb = this.repo.createQueryBuilder('event')
            .leftJoinAndSelect('event.mailbox', 'mailbox')
            .orderBy('event.timestamp', 'DESC')
            .take(limit);
        if (provider) {
            qb = qb.andWhere('TRIM(LOWER(event.provider)) = TRIM(LOWER(:provider))', { provider });
        }
        ...
        if (date) {
            ...
            qb = qb.andWhere('event.timestamp >= :start AND event.timestamp < :end', { start, end });
        }
        return qb.getMany();
    }
}
